...
test("desk commands append and consume from a cursor", async () => {
  ...
  try {
    ...
    const first = await enqueueDeskCommand(
      { action: "pause", sourceMarketId: "m1" },
      path,
    );
    ...
    assert.deepEqual(seen, [`pause:${first.id}`, `cancel:${second.id}`]);
...
test("skipExisting ignores commands written before the maker started", async () => {
  ...
  try {
    ...
    await skipExistingDeskCommands(path, cursorPath);
    const seen: string[] = [];
    assert.equal(await consumeDeskCommands(async (command) => {
