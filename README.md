...
打开 `http://127.0.0.1:3000`：配置页粘贴两个 URL → 预览并确认或交换队伍配对 → 设置额度/层数 → 保存并启动。运行中的市场配置会自动热加载，无需重启；交易台通过 SSE 实时显示连接、额度占用、计划/真实挂单、仓位和中文风控原因，也可热更新账户额度。`live` 模式仍要求 `.env` 中已有双重确认；停止按钮会向交易进程发送 `SIGTERM` 并执行保护性撤单。

KGL 可点击「一键发现 KGL」批量读取源站与 Polymarket 的开放比赛。系统按队伍和开赛时间配对，仅默认选择高置信且当前赔率允许安全抢队首的全场胜负盘口；确认后每边只挂一层 BUY，目标为买一加 1 tick，但不会突破源赔率安全上限或跨过卖一。
